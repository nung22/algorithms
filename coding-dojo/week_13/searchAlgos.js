...
function findObjects(criteria, collection) {
  // array to hold matched objects
  let results = []
  // iterate through collection of objects
  collection.forEach( nestedObj => {
    // tracks whether or not the current objects matches the search criteria
    let matched = true;
    // iterate through search criteria keys
    Object.keys(criteria).forEach( key => {
      // set matched to false if the object does not contain the search criteria
      if(!criteria.hasOwnProperty(key) || criteria[key] !== nestedObj[key]) {
        matched = false;
      }
    });
    // add the object to result array if it matched search criteria
    if(matched) {
      results.push(nestedObj);
    }
})
  return results;
}
